<div class="row packets_data">
    <div class="form-group col-md-4 select_2_error">
        ...
        <select class=" form-control select2 select_packets" style="text-transform: uppercase" onchange="findRemainnigShocks(this,{{ $index }})"  name="products[{{ $index }}]" required>
            ...
            @foreach($products as $product)
                <option value='{{ $product->style_code  }}'> ( {{  $product->print_packet_code ? $product->print_packet_code : $product->style_code }} ) {{ $product->brand_name }} - {{ $product->types_name }}  - {{ $product->size_name }}</option>
            @endforeach                         
        </select>
    </div>
    ...
    <div class="form-group col-md-2">
        <label class="form-label" for="rack_id">Full Packet Price</label>
        <input type="number" value="{{ $product->packet_sale_price }}" name="packet_price[{{ $index }}]" id="packet_price_{{ $index }}" class="form-control">        
    </div>
    <div class="form-group col-md-2">
