...
class AdminHomeController extends Controller
{
    ...
    /*----------------------------- Ramjan------------------------------- */
     /* ----------------------previous due bill -----------------------------------*/
      ...
      /*-------------------------- reack rifill--------------------------------------- */
       ...
       /*------------------------------ total rack -----------------------------------------*/
        //$socks_rack = DB::table('rack_mapping')->count('id');
         $socks_rack = DB::table('rack_mapping as rm')
      ...
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year'  and st.cat_id =1 ");

      $last_month_sale_socks = DB::select("SELECT count(*) as last_month_sale_socks from rack_products rp
      left join stocks st on rp.style_code = st.style_code
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and st.cat_id =1 ");

      $this_month_sale_socks_amt = DB::select("SELECT sum(selling_price) as   this_month_sale_socks_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year' and st.cat_id =1 ");

      $last_month_sale_socks_amt = DB::select("SELECT sum(selling_price) as last_month_sale_socks_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and st.cat_id =1");
      ...
      left join stocks st on rp.style_code = st.style_code
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year' and st.cat_id =2 ");

      $last_month_sale_tshirt = DB::select("SELECT count(*) as last_month_sale_tshirt from rack_products rp
      left join stocks st on rp.style_code = st.style_code
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and  st.cat_id =2 ");

      $this_month_sale_tshirt_amt = DB::select("SELECT sum(selling_price) as   this_month_sale_tshirt_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year' and st.cat_id =2 ");

      $last_month_sale_tshirt_amt = DB::select("SELECT sum(selling_price) as last_month_sale_tshirt_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and st.cat_id =2 ");
      ...
      left join stocks st on rp.style_code = st.style_code
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year' and st.cat_id =3 ");

      $last_month_sale_pant = DB::select("SELECT count(*) as last_month_sale_pant from rack_products rp
      left join stocks st on rp.style_code = st.style_code
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and st.cat_id =3 ");

      $this_month_sale_pant_amt = DB::select("SELECT sum(selling_price) as   this_month_sale_pant_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$current_month' and year(rp.sold_date)='$current_year' and st.cat_id =3 ");

      $last_month_sale_pant_amt = DB::select("SELECT sum(selling_price) as last_month_sale_pant_amt from rack_products rp
      left join stocks st on rp.style_code = st.style_code 
      where rp.status in  (7) and month(rp.sold_date)='$last_month' and year(rp.sold_date)='$current_year' and st.cat_id =3 ");
