...
class BillCollectionController extends Controller
{
    public function rackList()
    {
        ...
        if($role_id == 9)
        {
            $rack_list_sql = "rp.rack_code in ($rack_list->rack_codes)";
        }else{
                $rack_list_sql = "rp.rack_code <> '' ";
        }
    ...
    public function GetAllDue(Request $request)
    {
        ...
        $sql = "SELECT sum(month_sale_price) as monthly_due, shop_id, sum(rp.total_socks_pair) as total_socks_pair from (SELECT sold_date,shop_id, rack_code, sum(selling_price) as month_sale_price, count(*) as total_socks_pair from rack_products where status = '1' and rack_code= '$rack_code'  GROUP by sold_date) rp 
        ...
        if($due_month !='')
        {
            foreach($due_month as $single_month)
            {
               ...
               $total_socks += $single_month->total_socks_pair;
            }
    ...
    public function pay_all_due(Request $request)
    {
        ...
        // socks bill number generate
        $socks_bill_no = $this->generateShockBillNo($rack_code, $shop_id);



        // total sold year-month group by
        $sql = "SELECT 
                    sold_date, 
                    count(*) as sales_quantity, 
                    sum(selling_price) as collect_amount, 
                    agent_id, sum(shop_commission) as shop_commission_amount, 
                    sum(agent_commission) agent_commission_amount, 
        ...
        foreach($monthly_due_bill as $single_monthly_due_bill){
            ...
            $monthly_bill_calculate = $this->monthlyBillCollection($rack_code, $shop_id, $single_monthly_due_bill->sold_date); // for monthly bill table update

            // single month bill collection section 
              ...
              $flag= 'N';
               ...
               $updateMonthBillPayment = $this->updateMonthBillPayment($rack_code, $shop_id, $year, $month, $socks_bill_no, $agent_id, $sales_quantity, $collect_amount, $flag);
    ...
    // updated monthly bill table
    public function updateMonthBillPayment($rack_code, $shop_id, $year, $month, $socks_bill_no, $agent_id, $sales_quantity, $collect_amount, $flag){
        $sql = "SELECT 
        ...
        $monthly_billing_info  = DB::select($sql);
        $monthly_billing_data = $monthly_billing_info[0];

       


        $total_amount_payment = $monthly_billing_data->total_amount_payment; // current total bill 
        $total_shopkeeper_amount_payment = $monthly_billing_data->total_shopkeeper_amount_payment; // current total shopkeeper comission
        $total_agent_amount_payment = $monthly_billing_data->total_agent_amount_payment; // current total agent commission
        $total_venture_amount = $monthly_billing_data->total_venture_amount; // current total venture amount
        ...
        $old_bill_payment_sql = "SELECT * FROM `rack_monthly_bill` WHERE  billing_year_month='$billing_year_month' and  rack_code='$rack_code' and shop_id='$shop_id'"; 
        $old_bill_payment_data = DB::select(DB::raw($old_bill_payment_sql));

        // this month old payment information
        $old_data_id = $old_bill_payment_data[0]->id;
        $old_total_amount_payment = $old_bill_payment_data[0]->total_amount_payment; // old payment amount this month
        $old_total_shopkeeper_amount_payment = $old_bill_payment_data[0]->total_shopkeeper_amount_payment; // old payment shopkeeper amount this month
        $old_total_agent_amount_payment = $old_bill_payment_data[0]->total_agent_amount_payment; // old payment agent amount this month
        $old_total_venture_amount_payment = $old_bill_payment_data[0]->total_venture_amount_payment; // old payment venture amount this month


        // finding old sales commission 
        $old_sold_shop_commission_amount = ($old_bill_payment_data[0]->total_shopkeeper_amount - 
            ($old_total_shopkeeper_amount_payment  + $total_shopkeeper_amount_payment)
        );

        $old_sold_agent_commission_amount = ($old_bill_payment_data[0]->total_agent_amount - 
            ($old_total_agent_amount_payment  + $total_agent_amount_payment)
        );

        $old_sold_venture_commission_amount = ($old_bill_payment_data[0]->total_venture_amount - 
            ($old_total_venture_amount_payment  + $total_venture_amount)
        );




        // update monthly bill payment 
        DB::table('rack_monthly_bill')->where('id', $old_data_id)->update([
            "total_shopkeeper_amount_payment" => $old_bill_payment_data[0]->total_shopkeeper_amount,
            "total_agent_amount_payment" => $old_bill_payment_data[0]->total_agent_amount,
            "total_venture_amount_payment" => $old_bill_payment_data[0]->total_venture_amount,
            "total_amount_payment" => $old_bill_payment_data[0]->total_amount,
        ...
        // updated commission table for socks bill number
        DB::table('commissions')->insert([
            ...
            "total_amount"                => $total_amount_payment,
            "shop_commission_amount"      => $total_shopkeeper_amount_payment,
            "agent_commission_amount"     => $total_agent_amount_payment,
            "venture_amount"              => $total_venture_amount,
            "old_shop_commission"         => $old_sold_shop_commission_amount,
            "old_agent_commission"        => $old_sold_agent_commission_amount,            
            "old_venture_commission"      => $old_sold_venture_commission_amount,
        ...
        /*----------keep log for this month shocks code---------*/
        $log_data = DB::table('rack_products')->where([
            ['shop_id', '=', $shop_id],
            ['rack_code', '=', $rack_code]
        ])
        ->select('id')
        ...
        {
            $this->socksLog($single_log_data->id, 'SOCKS_BILL_COLLECTION_BY_AGENT');
        }
        ...
        try {
            ...
            $data = [
                    "status"   => 400,
                    "is_error" => true,
                    "message"  => "rack product   update fail"
                ];
            
        }

        
         // updated monthly socks bill table 
        $monthly_socks_bill = $this->updatedMonthlySocksBill($rack_code, $shop_id, $year, $month, $socks_bill_no, $agent_id, $sales_quantity, $collect_amount);
    ...
    // updated monthly socks bill table 
    public function updatedMonthlySocksBill($rack_code, $shop_id, $year, $month, $socks_bill_no, $agent_id, $sales_quantity, $collect_amount){

        $file_name = $socks_bill_no.'-bill.pdf';
        $location = "Bill/voucher/".$file_name;
        ...
        try {
            ...
            $data = [
                    "status"   => 400,
                    "is_error" => true,
                    "message"  => "shocks bill   insert  fail"
                ];
            
        }


      $this->generateBillVoucher($socks_bill_no);
...
#################################################### month wise bill collection contoller ##########
    ...
    public function all_month(Request $request)
    {
        $shop_id = $request->shop_id;

         $all_due_month = DB::Select("SELECT sold_date FROM `rack_products` where shop_id='$shop_id' and status='1' GROUP by month(sold_date), year(sold_date) ");

         $single_due_month = DB::Select("SELECT sold_date FROM `rack_products` where shop_id='$shop_id' and status='1' GROUP by month(sold_date), year(sold_date) order by sold_date asc limit 1 ");
         ...
         if(count($all_due_month) > 0)
         {
            ...
            $single_due_month = $single_due_month[0];

            $data = [

                'all_due_month' => $all_due_month,
                'single_due_month' => $single_due_month,
                'sl' => 1,
            ];
            $output = view('billCollection.due-month-list', $data);
    ...
    {
        $shop_id = $request->shop_id;
        $billing_month = $request->due_month;

        $month  = date('m', strtotime($billing_month));
        $year  = date('Y', strtotime($billing_month));

        $total_due = DB::table('rack_products')->where([
            'shop_id' => $shop_id,
            'status' => 1
        ])->whereMonth('sold_date', $month)->whereYear('sold_date', $year)->sum('selling_price');
        ...
        $total_socks = DB::table('rack_products')->where([
         ...
         $due_result  = DB::table('rack_products')
         ->select('rack_code')
         ->where([
            'shop_id' => $shop_id,
            'status' => 1
        ])->whereMonth('sold_date', $month)->whereYear('sold_date', $year)->first();

    
        if($total_due > 0)
        {
            $data = [
                'rack_code' => $due_result->rack_code,
                'total_due' => $total_due,
                'total_socks' => $total_socks
            ];
            return json_encode($data);
    ...
    public function monthlyBillPay(Request $request)
    {
        
        $bill_payment_month  = $request->due_month;

        $rack_code = $request->rack_code;
        $shop_id = $request->shop_id;
        $due_amount =  $request->bill_amount;
        $entry_user = Auth::user()->id;
        $entry_datetime = date('Y-m-d H:i:s');
        $current_date  = date('Y-m-d');

        $month = date('m', strtotime($bill_payment_month));
        $year = date('Y', strtotime($bill_payment_month));
        ...
        $sql = "SELECT 
                    ...
                    sum(venture_amount) as venture_amount  
                from rack_products 
                where rack_code='$rack_code' and month(sold_date)='$month' and year(sold_date) = '$year'
                and status ='1' 
                and shop_id='$shop_id' 
                GROUP by 
                month(sold_date), year(sold_date)";    
         ...
         foreach($monthly_due_bill as $single_monthly_due_bill){
              ...
              $monthly_bill_calculate = $this->monthlyBillCollection($rack_code, $shop_id, $single_monthly_due_bill->sold_date); //for monthly bill table 
...
############################################################## month wise bill collection contoller ##########
    ...
    public function generateShockBillNo($rack_code, $shop_id){
        ...
        $billno        = ($billSerial + 1).rand(1,999);
        $socks_bill_no = '900001'.$billno;
