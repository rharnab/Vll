...
class RackWrongSoldItemDeteController extends Controller
{
    public function rackList(){
        ...
        if($role_id == 9)
        {
            $rack_list_sql = " and rack_code in ($rack_list->rack_codes)";
        }else{
            $rack_list_sql = " ";
        }
         ...
         $shop_racks_sql = "SELECT
                    ...
                    FROM
                        ...
                        from
                            rack_products 
                        where
                              status=1 $agent_sql $rack_list_sql
                        GROUP by
                            shop_id,
                            rack_code
                        )
         ...
         $shop_racks_sql = "SELECT
                    ...
                    FROM
                        ...
                        from
                            rack_products 
                        where
                              status=1 and rack_code = '$rack_code'
                        GROUP by
                            shop_id
                        ...
                        shops s 
                        on rack.shop_id = s.id ";
        $shop_rack = DB::select(DB::raw($shop_racks_sql));

        $output  = view('agent.sold-delete.search-shop', compact('shop_rack'));
