...
class AgentRackController extends Controller
{
        ...
        foreach ($rack_style_sizes as $rack_style_size) {
            ...
            $shocks = DB::select(DB::raw("SELECT
                        ...
                        where
                            ...
                            and t.id = '$type_id' 
                            and (rp.status = 0 or rp.status = 2) order by rp.printed_socks_code asc"));
    ...
    public function calculateShocksBill(Request $request){
        if($request->has('shocks')){
            ...
            $products = DB::table('rack_products as rp')->select(['s.cat_id', 'shocks_code'])
            ->leftJoin('stocks as s', 's.style_code', 'rp.style_code')
            ->whereIn('rp.shocks_code', $shocks)->get();
            ...
            foreach($products as $single_product){
                ...
                $shocks_code = $single_product->shocks_code;
                $commissions      = $this->getTotalShocksCommissions($total_shocks, $shop_id, $cat_id);
                $agent_commission = $commissions['agent_commission_persentage'];
                $shop_commission  = $commissions['shop_commission_persentage'];
                $venture_commission = 100 - (0 + $shop_commission);

                $salling_price = $this->singleShockSellPrice($shocks_code);
                $shop_amount += $salling_price * ($shop_commission / 100);
                $venture_amount += $salling_price * ($venture_commission / 100);
                $total_sale_amout += $salling_price;
            ...
            $bill_collection_last_day = DB::table('collection_date')->first();
            ...
            $data = [
                ...
                "total_amount" => $total_sale_amout,
                "agent_amount" => $venture_amount,
                "shop_amount"  => $shop_amount,
                "bill_collection_last_day" => $bill_collection_last_day
            ];
