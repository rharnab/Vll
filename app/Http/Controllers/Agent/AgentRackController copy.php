...
';
    ...
    public function getCategories($shocks){
        //echo "<pre>";
        //print_r($shocks);
       $categories =  DB::table('rack_products as rp')->select(['s.cat_id'])
    ...
    public function CatWiseTotalSell($shocks, $cat_id){
        $totalSellPrice =  DB::table('rack_products as rp')
        ->leftJoin('stocks as s', 's.style_code', 'rp.style_code')
        ->whereIn('rp.shocks_code', $shocks)->where('s.cat_id', $cat_id)->sum('rp.selling_price');
