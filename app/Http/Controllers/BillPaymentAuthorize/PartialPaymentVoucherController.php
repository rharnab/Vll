...
use DB;
class PartialPaymentVoucherController extends Controller
{
    public function partialvoucherShow($voucher_no){
        $mpdf = new \Mpdf\Mpdf([
    ...
    public function pdfHTML($voucher_no){
             ...
             $rack_info = DB::table('shock_bills as sb')
                    ->select([
                        ...
                        'u.name as bill_generat_name'
                    ])
                    ->leftJoin('shops as s', 'sb.shop_id', 's.id')
                    ->leftJoin('agent_users as au', 'sb.agent_id', 'au.id')
                    ->leftJoin('users as u', 'u.id', 'sb.entry_user_id')
                    ->where('sb.shocks_bill_no', $voucher_no)
                    ->first();

            $partail_paid_amount= DB::table('partial_bill')->where('bill_no', $voucher_no)->where('status', 0)->sum('paid_amt');
...
<p>$total_unit  টি পণ্য  , সর্বমোট $total_bill_amt টাকা , দোকানদারের কমিশন $total_shop_commission $show_deu টাকা , দোকানদার দিবে $shop_given_amt টাকা   </p>";
...
$final_payment_due = $grand_shop_bill - $partail_paid_amount; 
...
<p> ***  পূর্বে জমাক্রিত টাকার পরিমান ".number_format($partail_paid_amount, 2)." টাকা বর্তামানে দোকানদার দিবে ".number_format($final_payment_due, 2)." টাকা ***  </p>
