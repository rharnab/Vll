...
class ShopDueBillReportController extends Controller
{
    public function index()
    {
        ...
        return view('report.shop-due-bill.index', $data);
    }
    ...
    {   
        ...
        $mpdf = new \Mpdf\Mpdf([
         ...
         $mpdf->WriteHTML($this->storeVoucher($rack_code));

        $download_file_name=$rack_code." Bill voucher ".date('Y-m-d h:i:s a').".pdf";
        $mpdf->Output("$download_file_name", 'I');
    }

    
    public function storeVoucher($rack_code){
        ...
        where rm.rack_code = '$rack_code' ")[0]; */

        $rack_info = DB::table('rack_products as rp')
        ->select(['s.name  as shop_name', 's.area  as shop_area','s.market_name', 'rp.rack_code', 'au.name  as agent_name' ])
        ->leftJoin('shops as s', 's.id', 'rp.shop_id')
        ->leftJoin('agent_users as au', 'au.id', 'rp.agent_id')
...
<h3 style='text-align:center;'>সকল বকেয়া  পরিমাণ  /   All due bill  </h3>
  <table>
...
$billing_month_sql  = "SELECT sold_date FROM  rack_products where STATUS=1 and rack_code ='$rack_code' GROUP by month(sold_date), year(sold_date) order by sold_date asc";
...
$billing_year_month = $year.'-'.$month;
      ...
      0 as shop_commission_amount,
      0 as old_shop_commission,
      0 as due_shop_commission
      FROM   (SELECT rp.shocks_bill_no    AS rp_socks_bill,
             ...
             LEFT JOIN rack_monthly_bill rmb
                    ON rmb.billing_year_month = '$billing_year_month' 
      WHERE  rmb.billing_year_month = '$billing_year_month'
      GROUP  BY r.type_id ";
