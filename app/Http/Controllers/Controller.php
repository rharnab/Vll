...
class Controller extends BaseController
{
    ...
    public function getTotalShocksCommissions($total_shocks, $shop_id, $cat_id){
        $checkDataCount = DB::table('commission_setups')->where('shop_id', $shop_id)->where('cat_id', $cat_id)->count();
        if($checkDataCount > 0){
            ...
            )->where('statring_range', '<=', $total_shocks)->where('ending_range', '>=', $total_shocks)
            ->where('cat_id', $cat_id)
            ->where('shop_id', $shop_id)
            ->first();
        }else{
            $shocks_commission = DB::table('commission_setups')->select('agent_commission_persentage', 'shop_commission_persentage')->where('statring_range', '<=', $total_shocks)->where('ending_range', '>=', $total_shocks) ->where('cat_id', $cat_id)->first();
        }
    ...
    public function generateQrCode($rack_code, $qr_text){
        $folder_path = "public/qr-code/$rack_code/";
        if(!File::isDirectory($folder_path)){
            File::makeDirectory($folder_path, 0777, true, true);
        }
        $file = $folder_path.$qr_text.".png";
        QRCode::text($qr_text)->setSize(8)->setMargin(2)->setOutfile($file)->png();
        return $file;
    }
    ...
    /*-------------------------------bill Voucher store---------------------------------*/
    public function generateBillVoucher($voucher_no){
        $mpdf = new \Mpdf\Mpdf([
            'default_font_size'=>10,
            'default_font'=>'nikosh'
        ]);

         $mpdf->WriteHTML($this->storeVoucher($voucher_no));

        $file_name = $voucher_no.'-bill.pdf';

        $path = public_path('Bill/voucher');
    ...
    public function storeVoucher($voucher_no){
            
           $rack_info = DB::select("SELECT sb.entry_datetime, s.name  as shop_name, s.area  as shop_area, s.market_name, sb.rack_code, au.name  as agent_name, u.name as bill_generat_name  FROM  shock_bills sb 
                left join rack_mapping rm on rm.shop_id = sb.shop_id
                left join shops s on s.id = sb.shop_id
                left JOIN agent_users  au on au.id = sb.agent_id
                left JOIN users  u on u.id = sb.entry_user_id
                where sb.shocks_bill_no= '$voucher_no' ")[0];
            ...
            $bill_generat_by     = $rack_info->bill_generat_name;
...
<tr>
       <td> ভাউচার  তৈরি  / Voucher Generate By. <span style='color:red;'>*</span></td>
       <td>$bill_generat_by</td> 
    </tr>
...
$net_amount = $single_sold_info->total_bill -  $single_sold_info->total_commission;
$net_amount = number_format($net_amount, 2);

$shop_commission_parcent= ($single_sold_info->total_commission * 100) / $single_sold_info->total_bill;


$total_net_amount =  $total_net_amount + str_replace(',', '', $net_amount);
$shop_given_amt = $total_net_amount - $single_sold_info->due_shop_commission;
...
<td>".number_format($single_sold_info->total_commission, 2).'/='." (".$shop_commission_parcent."%)"."</td>
 <td>".$net_amount." /=-</td>         
</tr>";
...
<p>$total_unit টি, সর্বমোট $total_bill_amt টাকা , দোকানদারের কমিশন $total_shop_commission $show_deu টাকা , দোকানদার দিবে $shop_given_amt টাকা   </p>";
...
<p> ***  $grnad_total_shocks টি পণ্য  ,  সর্বমোট $grnad_total_bill টাকা , দোকানদারের কমিশন $grnad_shop_commission টাকা , দোকানদার দিবে $grand_shop_bill টাকা ***  </p>
