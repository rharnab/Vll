...
{   
    ...
    public function socks_code_generate2(){
         $get_rack_info = DB::table('racks')->get();
        return view('report.rack-socks-code.index2', compact('get_rack_info'));
    }
    ...
    public function generate_pdf(Request $request){
         ...
         $date =  $request->date;
           ...
           $get_data=DB::select(DB::raw("SELECT rack.brand_id,rack.printed_socks_code,rack.selling_price,rack.shop_socks_code,b.short_code as brand_name, rack.sl_priority,rack.rp_id
          FROM (SELECT st.brand_id,rp.printed_socks_code,rp.shop_socks_code,rp.selling_price,t.sl_priority,rp.id as rp_id FROM `rack_products` rp 
                LEFT JOIN stocks st on rp.style_code = st.style_code
                LEFT JOIN types t on t.id = st.type_id
                WHERE rp.rack_code='$rack_code' AND (rp.status='0' or rp.status='2')) rack 
          LEFT JOIN brands b on rack.brand_id = b.id
          order by rack.sl_priority,rack.rp_id asc"));

        

       $refill_check = DB::select(DB::raw("SELECT count(*) as total  from (SELECT entry_date FROM `rack_products` WHERE rack_code='$rack_code' GROUP by entry_date) fill_count"));
       
       $shop_info=DB::select(DB::raw("SELECT sh.name as shop_name, sh.shop_address, sh.contact_no  FROM `rack_products` rp left join shops sh on rp.shop_id=sh.id  WHERE rp.rack_code='$rack_code'"))[0];
        
        $refil_date = DB::select(DB::raw("SELECT entry_date FROM `rack_products` WHERE rack_code='$rack_code' GROUP by entry_date ORDER by entry_date DESC"));
        
      $data = [
          "get_data" => $get_data,
          "rack_no"  => $rack_code,
          "price_tag" => $price_tag,
          "date"     => $refil_date[0]->entry_date,
          "refill_count" => $refill_check[0]->total,
          "shop_info" => $shop_info,
      ];
        ...
        return $pdf->stream($shop_info->shop_name."_socks_code.pdf", array("Attachment" => false));
        ...
        public function generate_pdf2(Request $request){
         ...
         $date =  $request->date;
          ...
          FROM (SELECT st.brand_id,rp.printed_socks_code,rp.shop_socks_code,rp.selling_price,t.sl_priority,rp.id as rp_id FROM `rack_products` rp 
                LEFT JOIN stocks st on rp.style_code = st.style_code
                LEFT JOIN types t on t.id = st.type_id
                WHERE rp.rack_code='$rack_code' and rp.entry_date  BETWEEN '$finally_date' and '$ending_date' AND (rp.status='0' or rp.status='2')) rack 
          LEFT JOIN brands b on rack.brand_id = b.id
          order by rack.sl_priority,rack.rp_id asc"));
...
<img src='data:image/png;base64,'/>
    ...
    public function bill_authorize_report(){

      return view('report.bill-authorize-report.index');

    }

    public function bill_authorize_report_details(Request $request){
      ...
      $sql ="SELECT bi.*,
            ...
            FROM   (SELECT sb.shop_id,
                            sb.agent_id,
                            sb.auth_user_id,
                            sb.auth_datetime,
                            sb.shocks_bill_no                  AS bill_no,
                            sb.shocks_bill_no, 
                            (SELECT Sum(quantity)
                            FROM   commissions c
                            WHERE  c.shoks_bill_no = bill_no) AS total_socks,
                            (SELECT Sum(total_amount)
                            FROM   commissions c
                            WHERE  c.shoks_bill_no = bill_no) AS total_bill,
                            (SELECT Sum(shop_commission_amount)
                            FROM   commissions c
                            WHERE  c.shoks_bill_no = bill_no) AS total_shop_commission,
                            (SELECT Sum(agent_commission_amount)
                            FROM   commissions c
                            WHERE  c.shoks_bill_no = bill_no) AS total_agent_commission,
                            (SELECT Sum(venture_amount)
                            FROM   commissions c
                            WHERE  c.shoks_bill_no = bill_no) AS total_venture_amt,
                            (SELECT sum(amount)  from monthly_rack_conveynce_bill mrcb  where bill_no = sb.shocks_bill_no) as total_convenience,
                            (select sum(buying_price) from rack_products rp where shocks_bill_no = sb.shocks_bill_no) as factory_amount
                    FROM   shock_bills sb
                    WHERE  Date(sb.auth_datetime) BETWEEN '$start_date' AND '$end_date'
                            
                    GROUP  BY sb.shocks_bill_no) bi
                    LEFT JOIN shops s
                          ON s.id = bi.shop_id
                    LEFT JOIN users u
                          ON u.id = bi.auth_user_id
                    LEFT JOIN agent_users au
                          ON au.id = bi.agent_id ";
  
  $get_data =  DB::select(DB::raw($sql));

  $sum_result=DB::select("SELECT Sum(si.total_bill)             AS grand_total_bill,
          Sum(si.total_shop_commission)  AS grand_shop_commission,
          Sum(si.total_agent_commission) AS grand_agent_commission,
          Sum(si.total_venture_amt)      AS grand_venture_amt,
          Sum(si.total_socks)      AS grand_total_socks
    FROM   (SELECT bi.*,
                  s.NAME  AS shop_name,
                  u.NAME  AS auth_name,
                  au.NAME AS agent_name
      ...
      return view('report.bill-authorize-report.details', $data);

    } // end bill_authorize_report_details function


    public function socks_log_report(){

      
      return view('report.sock-log-report.index');

    }

    public function socks_log_report_submit(Request $request){
      
      $socks_code = $request->socks_code;
      $get_data = DB::select(DB::raw("SELECT rp.agent_id,au.name as agent_name, rp.shop_id,sh.name as shop_name, rp.rack_code,rp.type_id, rp.printed_socks_code, rp.status,rp.buying_price, rp.selling_price, sl.message,sl.operation_datetime,  t.types_name  FROM `rack_products` rp   LEFT JOIN socks_log sl on rp.id = sl.socks_code LEFT JOIN agent_users au on rp.agent_id=au.id 
      LEFT JOIN shops sh on rp.shop_id=sh.id 
      LEFT JOIN types t on rp.type_id=t.id
      WHERE rp.`printed_socks_code` = '$socks_code'"));

     
      return view('report.sock-log-report.details', compact('get_data'));
