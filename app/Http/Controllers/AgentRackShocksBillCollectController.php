        ...
        if($request->has('shocks')){
            ...
            $entry_datetime = date('Y-m-d H:i:s');
             ...
             if(date('d') > $bill_col_last_day){
                ...
                $skip_month    = date('Y-m', strtotime('-2 month', strtotime($current_month))); 

                if( (date('Y-m', strtotime($sold_date)) <= $skip_month) || (date('Y-m', strtotime($sold_date)) == $current_month))
                {
                    $data = [
                        "status"   => 400,
                        "is_error" => true,
                        "message"  => "Please select only last month",
                    ];
                    return response()->json($data);
                }
               /*  $current_month = date('Y-m');
                $sold_date    = date('Y-m-t', strtotime('-1 month', strtotime($current_month))); */
            ...
            /*-- find out all updated  category product ----------- */
