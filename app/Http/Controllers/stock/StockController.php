...
class StockController extends Controller
{
    ...
    public function ProductCheck(Request $request)
    {
        ...
        $size_check = $request->size_check;
         ...
         $product_details = DB::select("SELECT id, individual_selling_price FROM products where brand_id='$brand_check' and brand_size_id='$size_check' and type_id='$type_check' and packet_socks_pair_quantity='$per_pkt_qty' order by id desc limit 1 ");

         //echo "SELECT id, packet_selling_price FROM products where brand_id='$brand_check' and brand_size_id='$size_check' and type_id='$type_check' and packet_socks_pair_quantity='$per_pkt_qty' order by id desc limit 1 ";die;
         ...
         if(!empty($product_details[0]))
         {
            ...
            $data =[
                'success' => 1,
                'sale_price' => $product_details[0]->individual_selling_price,
            ];
